...
Peer_Handler<PEER_STREAM>::Peer_Handler(ACE_Reactor *r) : action_(&Peer_Handler<PEER_STREAM>::uninitialized)
{
    this->reactor(r);
}
...
template <typename PEER_STREAM> int
Peer_Handler<PEER_STREAM>::handle_timeout(const ACE_Time_Value &, const void *)
{
...
template <typename PEER_STREAM> int
Peer_Handler<PEER_STREAM>::handle_close(ACE_HANDLE h, ACE_Reactor_Mask mask)
{
    ACE_DEBUG((LM_DEBUG,
...
IPC_Client<SVC_HANDLER, PEER_CONNECTOR>::IPC_Client(void) : done_handler_(ACE_Sig_Handler_Ex(ACE_Reactor::end_event_loop))
{
}
...
IPC_Client<SVC_HANDLER, PEER_CONNECTOR>::init(int argc, ACE_TCHAR *argv[])
{
    ...
    const ACE_TCHAR *r_addr = argc > 1 ? argv[1] : ACE_SERVER_ADDRESS(ACE_DEFAULT_SERVER_HOST, ACE_DEFAULT_SERVER_PORT_STR);
    ...
    SVC_HANDLER *sh;
    ACE_NEW_RETURN(sh, SVC_HANDLER(this->reactor()), -1);
