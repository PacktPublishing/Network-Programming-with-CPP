...
Svc_Handler<PEER_STREAM>::Svc_Handler(ACE_Reactor *r) : SVC_HANDLER(0, 0, r)
{
}
...
template <typename PEER_STREAM> int
Svc_Handler<PEER_STREAM>::handle_timeout(const ACE_Time_Value &, const void *)
{
...
{
    const ACE_TCHAR *local_addr = argc > 1 ? argv[1] : ACE_DEFAULT_SERVER_PORT_STR;
    ...
    // initialization.
    if (this->inherited::open(this->server_addr_, use_reactor ? ACE_Reactor::instance() : 0) == -1)
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%p\n"),
                          ACE_TEXT("open")),
                          -1);
    // Handle the SIGINT signal through the <ACE_Reactor>.
    else if (ACE_Reactor::instance()->register_handler(SIGINT, &this->done_handler_) == -1)
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%p\n"),
    ...
    else if (ACE_Reactor::instance()->register_handler(SIGPIPE, &this->done_handler_) == -1)
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%p\n"),
                          ACE_TEXT("register_handler")),
                          -1);
...
template <typename SVC_HANDLER, typename PEER_ACCEPTOR> int
IPC_Server<SVC_HANDLER, PEER_ACCEPTOR>::handle_close(ACE_HANDLE handle, ACE_Reactor_Mask mask)
{
    ACE_UNUSED_ARG(handle);
    ACE_UNUSED_ARG(mask);

    ACE_DEBUG((LM_DEBUG, ACE_TEXT("calling IPC_Server handle_close, but accept handle stays open!\n")));
    return 0;
}
